...
側の window 設定で決まり、その手前のどこに帯を置くかは利用者が `used_percent` で決める。
...
帯の位置 (`used_percent`) は同梱テンプレでは両者同じ (20/40/60/80/90/95/97 と 20/40/60/80/90)。
...
- どの % で auto compact が走るかは plugin からは分からない。on 側の帯は固定 % なので、
  発火点の手前で鳴らしたいユーザは自分で `used_percent` を調整する
